...
import style from "./PaginatedFavorites.module.css";
import { useSelector } from "react-redux";

export default function PaginatedFavorites({ pageFunction, data, current }) {
  const allDogs = useSelector((state) => state.dogsFavorites);
    ...
    if (pages < maxPageDisplayed + 1 && pages > minPageDisplayed - 1) {
      return (
        <li
          key={pages}
          id={pages}
          className={current === pages ? style.active : null}
          onClick={() => handleClik(pages)}
        >
  ...
  return (
    <nav className={style.all}>
      ...
      <ul className={style.listaPag}>{renderPageNumber}</ul>

      <button
        className={style.next}
        onClick={(e) => next(e, current)}
